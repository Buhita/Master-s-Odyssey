<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="Editar_partidaR.aspx.cs" Inherits="Masters_Odyssey.Editar_partidaR" %>
...
<body class="indigo lighten-5">
    ...
    <header style="text-align: right; margin-right: 10px">
      ...
      <h2 align="center">Master's Odyssey</h2>
        ...
        <div class="container" style="align-content:center" id="div_per">
            <h5>Edición de partida realizada</h5>
        <br/>
            <form id="form1" runat="server">
                ...
                <table id="tabpar">
                    ...
                    <tr>
                        <td colspan="2"><h6>Resumen: </h6></td>
                        <td colspan="2"><h6>Notas: </h6></td>
                    </tr>
                    <tr>
                        <td colspan="2" rowspan="4"><textarea id="txt_res" class="materialize-textarea" maxlength="1000" style="height: 400px;" name="txt_res"></textarea></td>
                        <td colspan="2" rowspan="4"><textarea id="txt_not" class="materialize-textarea" maxlength="1000" style="height: 400px;" name="txt_not"></textarea></td>
                    </tr>
                </table>
                <table>
                    <tr>
                        <td  colspan="4">
                            <input type="submit" name="btngua" value="Guardar partida" />
                        </td>
                    </tr>
                </table>
                </div>
    ...
    <a href="Partidas_realizadas.aspx" style="padding-left:100px;"><--Volver</a>
    <%
        ...
        if (Request.QueryString["id"] != null)
        {
            ...
            foreach (var item in listapr)
            {
                if (item.Id.Equals(id))
                {
                    ...
                    Response.Write("<script>document.getElementById('txt_res').value ='" + item.Resumen + "';</script>");
        ...
        if (!string.IsNullOrEmpty(Request.Form["btngua"]))
        {
            ...
            if (Request.QueryString["id"] != null)
            {
                ...
                if (res.Length > 1000)
                {
                    error = error + "El resumen debe tener hasta 1000 caracteres. ";
                }
                if (not.Length > 1000)
                {
                ...
                if (error.Length > 0)
                {
                    ...
                    Response.Write("<script>document.getElementById('txt_res').value ='" + res + "';</script>");
                ...
                else
                {
                    ...
                    foreach (var item in listapr)
                    {
                        if (item.Id.Equals(id))
                        {
                            p = new Partida(id,nom,res,not,int.Parse(cam),1);
                            string mensaje = d.ModificarPartidaRealizada(p);
                            Response.Write("<script>alert('"+mensaje+"');</script>");
                            Response.Write("<script>location.href='Partidas_realizadas.aspx?id="+item.Id+"';</script>");
                        }
                    }
                }
            ...
            else
            {
                ...
                {
                    DAO d = new DAO();
                    p = new Partida(0,nom,res,not,int.Parse(cam),1);
                    string mensaje = d.CrearPartidaRealizada(p);
                    Response.Write("<script>alert('"+mensaje+"');</script>");
                    Response.Write("<script>location.href='Partidas_realizadas.aspx';</script>");
                }
            }
        }
    %>
